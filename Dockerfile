...
ENV EXIM_VERSION=4.93-r1
ENV BUILD_DATE=2020-05-22
...
RUN apk update && apk upgrade && apk add --no-cache tzdata && cp /usr/share/zoneinfo/Europe/Rome /etc/localtime && \
    apk add --no-cache tini exim exim-cdb exim-dbmdb exim-dnsdb exim-postgresql exim-scripts exim-sqlite exim-utils redis libressl openssl && \
